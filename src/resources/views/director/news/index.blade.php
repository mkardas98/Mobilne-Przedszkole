        ...
        <span class="pageNavigation__title">
           Aktualności
            ...
            <div class="pageNavigation__buttons">
                <a href="{{route('director.news.edit')}}" class="primaryButton">Dodaj nową aktualność</a>
            </div>
        </div>
                        ...
                        <table id="dataTable" class="ui celled table">
                            ...
                            @foreach($items as $key=>$item)
                                <tr>
                                    ...
                                    <td><small>
                                            @if($item->status)
                                                <a href="{{route('news.show.'.$item->id)}}" title="{{$item->title}}" target="_blank">{{$item->seo->seo_url}}</a>
                                                @else
                                                {{$item->seo->seo_url}}
                                                @endif
                                        </small></td>
                                    ...
                                    <td class="tableButtons">
                                        <a class="controlButton -blue" href="{{route('director.news.edit', ['id'=>$item->id])}}"><i class="far fa-edit"></i></a>
