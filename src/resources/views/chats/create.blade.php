                       ...
                       @if(count($users)>0)
                            <form action="{{route('chats.create')}}" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="message" required class="form-control" id="message" rows="10" placeholder="Wprowadź treść wiadomości"></textarea>
                                </div>
                                <button class="primaryButton -green" type="submit">Wyślij wiadomość</button>
                            </form>
                        @else
                           Masz już rozpoczęte konwersację z wszystkimi dostępnymi dla Ciebie użytkownikami.
                        @endif
                    </div>
                </div>
            </div>
